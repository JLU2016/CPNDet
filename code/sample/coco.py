...
def kp_detection(db, k_ind, data_aug, debug):
    ...
    gt_bboxes    = np.zeros((batch_size, max_tag_len, 5), dtype=np.float32)
    ...
    for b_ind in range(batch_size):
        ...
        for ind, detection in enumerate(detections):
            ...
            gt_bboxes[b_ind, tag_ind, :]  = [xtl, ytl, xbr, ybr, category]
    ...
    tag_lens    = torch.from_numpy(tag_lens)

    return {
        "xs": [images, tl_tags, br_tags, gt_bboxes, tag_lens],
